...
| 4 | 백테스트: 다음 캔들 시가 체결, 수수료·슬리피지, 손절·익절, 성과 지표(수익률·연환산·MDD·Sharpe·승률·PF·거래당 평균 수익률·실현 손익), Buy & Hold 비교, 결과 저장 | **완료** |
...
.venv\Scripts\python.exe -m app.main backtest KRW-BTC --interval 240m --start 2025-01-01 --strategy ichimoku                # 신규 전략 (기본 파라미터)
.venv\Scripts\python.exe -m app.main backtest KRW-BTC --interval 60m --start 2025-01-01 --strategy volume_breakout --stop-loss 0.05 --trailing-stop 0.08
...
- **연도별 최적 설정** (설정 탭, 전략과 캔들 단위 사이의 `2025년 … 2021년` 버튼 — 올해를 뺀 최근 5년): 누른 해(1/1~12/31 KST)의 실제 캔들로 캔들 단위(60m·240m·1d) × 그 전략의 파라미터 후보(기본값 포함, 4~18개) × 청산 규칙 8가지(끔·손절 3/7%·추적 손절 5/10/20%·손절 5%+익절 10%·손절 7%+추적 15%)를 모두 백테스트해, 폼의 마켓 **평균 수익률이 가장 높은 조합**을 폼에 채운다(캔들 단위·파라미터·손절/익절/추적 손절, 변경 메모 포함). 어떤 조합도 단순 보유보다 낮으면 그중 가장 높은 조합을 고르고 그렇다고 표시한다. 수수료·슬리피지·초기 자본은 백테스트 탭 값, 나머지 리스크(현금 비율·일일/연속 손실 한도 등)는 폼 값을 그대로 쓴다. 결과를 채울 때 백테스트 탭의 "리스크 규칙" 도 켜 두므로 같은 해를 돌리면 같은 수익률이 나온다 — 이 수익률은 손절·익절이 들어간 값이라 리스크 규칙을 끄고 돌리면 다르게 나온다(끈 채 설정에 손절·익절이 있으면 백테스트 탭이 경고한다). 결과에는 **같은 설정의 다음 해 성과**도 함께 나온다(지난 한 해에 맞춘 값이 다음 해에도 통했는지 — 과최적화 확인용). 다음 해가 올해면 1/1~오늘 0시까지로 계산해 하루 동안 결과를 재사용하므로, 지금 시각까지 보는 백테스트 탭의 올해 결과와 조금 다를 수 있다. 저장은 하지 않으므로 확인 후 "저장" 을 눌러야 봇에 적용된다. 캔들은 백테스트 탭과 같은 캐시(`data/cache`)를 쓰고, 결과는 `data/optimize/` 에 저장돼 같은 조건으로 다시 누르면 바로 나온다(처음은 캔들 조회 때문에 1~2분, 계산은 전략·마켓당 수 초). 한 번에 한 작업만 돌고 "중단" 할 수 있다.
- **로그 탭**: 최근 100개만 먼저 읽고, 아래로 내리거나 "이전 100개 더 보기"로 이전 로그를 이어서 불러온다(`before_id` 커서). 레벨·날짜 범위(KST)·이벤트/메시지 검색으로 거를 수 있다.
- **백테스트 탭**: 설정 탭의 전략·파라미터로 과거 캔들 시뮬레이션을 돌린다(실제 주문 없음, 저장하지 않고 바꾼 폼 값도 바로 반영). 마켓·캔들 단위도 설정 탭 값을 따라가고, 백테스트 탭에서 직접 바꾸면 그 값은 이 화면에서만 쓰이다가 "현재 설정과 같게"를 누르면 다시 설정을 따라간다. 마켓(코인 검색 팝업)·캔들 단위·기간(연도별 체크, 최근 3/6/12개월, 직접 지정 — 여러 개 고르면 구간 × 마켓 조합을 각각 돌려 비교)·초기 자본·**거래소 수수료(편도 %)**·슬리피지·리스크 규칙 적용 여부를 정한다. 작업은 백그라운드로 돌고 진행률이 표시되며, 결과 표(전략 수익률·단순 보유·MDD·거래·승률·거래당 평균·PF·수수료 합계)에서 행을 누르면 돌린 파라미터, 자산 곡선(전략 vs 단순 보유)·지표(실현 손익·거래당 평균 수익률 포함)·거래 내역이 보인다. 다른 전략을 시험하려면 설정 탭에서 전략을 바꾼 뒤(저장 전 폼 값도 그대로 쓰인다) 다시 실행한다. 결과는 DB(`backtest_jobs`, 최근 100개)에 저장되어 서버를 다시 시작해도 "이전 결과 보기" 목록에서 다시 볼 수 있고(삭제 가능), 파일로도 `data/backtests/dash_*` 에 CLI 와 같은 형식(summary.json, trades.csv, equity.csv, signals.csv)으로 남는다.
...
│   │   ├── optimize.py         # 연도별 최적 설정 탐색: 전략별 파라미터 후보·청산 규칙 8종, 조합 전체 백테스트 → 최고 수익률
...
│   │   ├── optimizer.py        # 대시보드 '연도별 최적 설정' 실행기: 그 해 캔들 로드 → 탐색(스레드) → 다음 해 확인 → data/optimize 캐시
...
새 전략 추가: `Strategy` 상속 → `name`, `description`, `family`(계열), `rules`(화면용 규칙 요약), `Params`, `warmup_periods`, `evaluate()` 구현 → `app/strategy/__init__.py` 의 `STRATEGIES` 에 등록. 파라미터를 `param(기본값, label=..., description=..., help_text=..., depends_on=...)` 으로 선언하면 대시보드 폼에 이름·설명·도움말이 그대로 나온다. 등록만 하면 공통 테스트(Look-ahead·워밍업·빈 입력·수수료 반영 백테스트)가 자동 적용되고 설정 탭 드롭다운에 나타난다.
...
- 참고(KRW-BTC 60분봉 2025년 한 해를 전략마다 기본 파라미터로 백테스트, 수수료·슬리피지 각 0.05%, 리스크 규칙 없음): `bollinger` 만 +8.3%, 나머지는 −7%(`ma_cross`) ~ −66%(`obv`), 단순 보유 −9.9%. 거래가 잦은 전략(`obv` 연 439회, `williams_r` 220회)은 수수료·슬리피지만으로 자본의 15~29% 가 빠졌다 — 짧은 캔들에서는 긴 캔들·긴 기간 파라미터·리스크 규칙과 함께 검증할 것. 한 구간·한 마켓의 결과일 뿐이다.
...
6. 수익률·실현 손익·거래당 평균 수익률은 모두 수수료·슬리피지를 뺀 값이고, 수수료 합계도 따로 보여 준다(테스트가 등록된 모든 전략에 대해 확인).
...
3. 캔들 확정: 캔들 경계 + `CANDLE_GRACE_SECONDS` 마다 REST 로 최근 캔들을 다시 받아 **닫힌 캔들만** 합친다. 주(`1w`)·월(`1M`)·연(`1y`) 캔들은 경계가 UTC 고정 간격으로 정렬되지 않아 엔진에서 지원하지 않는다(설정 오류로 시작 거부, 백테스트만 가능). WebSocket 캔들 스트림은 체결이 있을 때만 오고 재연결 중 빠질 수 있어 확정 기준으로 쓰지 않는다.
